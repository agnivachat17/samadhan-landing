...
import { OrganizationStatus, OrganizationStatusLoading } from "@/components/OrganizationStatus";
...
export default function ProtectedRoute({
  ...
  const isOrgRole = me.data?.role === "institution" || me.data?.role === "industry";
  const shouldCheckVerification = requireVerifiedOrganization && isOrgRole && !!me.data?.organizationId;
  ...
  useEffect(() => {
    if (roles && me.data && !roles.includes(me.data.role)) {
      setLocation(dashboardPathForRole(me.data.role, me.data.organizationId ?? null));
    }
  ...
  if (loading || !user || me.isLoading || roleMismatch) {
    return <div className="grid min-h-[60vh] place-items-center font-body text-sm text-[#50655b]">Checking your session…</div>;
  }
    ...
    if (
      ...
      return (
        <main className="min-h-screen bg-[#f1eadc] text-[#0d3024]" style={{ backgroundImage: "url('/manus-storage/samadhan-paper-grain_46302c3f.jpg')", backgroundSize: "cover" }}>
          {kind === "institution" ? <InstituteHeader active="Dashboard" /> : <IndustryHeader />}
