/** Style: Samadhan project governance ledger with Firestore-backed delivery records. */
    ...
    () =>
      (projectsQuery.data ?? []).filter(
        project =>
          ...
          `${project.title} ${organizations.find(org => org.id === project.organizationId)?.name ?? ""}`
            .toLowerCase()
            .includes(query.toLowerCase())
      ),
    [projectsQuery.data, organizations, status, query]
  );
  return (
    <main
    ...
    >
      ...
      <section className="px-6 py-10 sm:px-10 lg:px-[2.8rem] lg:py-10">
        <div className="mx-auto max-w-[96rem]">
          <div className="border-b border-[#a78e6e]/45 pb-7">
            ...
            <p className="mt-4 font-body text-[0.9rem] text-[#53675d]">
              Monitor institute delivery, closeout readiness, evidence, and
              risks across persisted project records.
            </p>
          </div>
          <div className="mt-6 grid gap-4 sm:grid-cols-[1.5fr_.7fr]">
            <label className="flex h-[3.48rem] items-center gap-3 border border-[#a58c6d]/55 bg-[#f8f2e8]/28 px-4">
              <Search size={21} />
              <input
                value={query}
                onChange={event => setQuery(event.target.value)}
                placeholder="Search project or institution…"
                className="min-w-0 flex-1 bg-transparent font-body text-[0.8rem] outline-none"
              />
            </label>
          ...
          <div className="mt-6 hidden grid-cols-[minmax(20rem,1.45fr)_.95fr_.75fr_.95fr_.65fr_7rem] gap-4 border-b border-[#a78e6e]/40 pb-4 font-mono-ui text-[0.59rem] font-semibold uppercase tracking-[0.1em] text-[#314a40] lg:grid">
            <span>Project</span>
          ...
          closeoutsQuery.isLoading ? (
            ...
            <div>
              {visible.map(project => {
                ...
                const closeout = (closeoutsQuery.data ?? []).find(
                  item => item.projectId === project.id
                );
                return (
                  <article
                    key={project.id}
                    className="grid gap-4 border-b border-[#a78e6e]/40 py-5 lg:grid-cols-[minmax(20rem,1.45fr)_.95fr_.75fr_.95fr_.65fr_7rem] lg:items-center lg:gap-4 lg:px-3"
                  >
                    <div>
                      <a
                        ...
                        className="group"
                      >
                        <h2 className="font-display text-[1.28rem] leading-none group-hover:text-[#b84a28] sm:text-[1.45rem]">
                          {project.title}
                        </h2>
                      </a>
                      {project.riskSummary && (
                        <p className="mt-2 font-body text-[0.7rem] text-[#b54a28]">
                          Risk: {project.riskSummary}
                        </p>
                      )}
                    </div>
                    <p className="font-body text-[0.74rem] leading-snug text-[#354e43]">
                      {institution?.name ?? "Institution record"}
                    </p>
                    <p className="font-body text-[0.75rem]">
                      {project.stage.replaceAll("_", " ")}
                    </p>
                    <div className="flex items-center gap-3">
                      <span className="w-8 font-body text-[0.76rem] font-semibold">
                        {project.progress}%
                      </span>
                      <span className="h-1 flex-1 bg-[#d8cfbd]">
                        <span
                          className="block h-full bg-[#c64b22]"
...
function Loading() {
  return (
    <div className="mt-7 flex items-center gap-3 p-6 font-body text-[#52675d]">
      ...
      Loading project governance records…
    </div>
  );
}
function Empty() {
  return (
    <div className="mt-7 border border-dashed border-[#a58c6d]/55 p-8 text-center font-body text-[0.8rem] text-[#586d63]">
      No matching persisted projects.
    </div>
  );
}
