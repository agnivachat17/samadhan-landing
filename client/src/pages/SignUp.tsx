...
> = {
  citizen: {
    title: "Citizen",
    description: "Report and follow local challenges",
    next: "Create citizen account",
  },
  institution: {
    title: "Institute",
    description: "Contribute faculty, students, and facilities",
    next: "Continue to institute application",
  },
  industry: {
    title: "Industry",
    description: "Offer CSR, capability, and deployment support",
    next: "Continue to industry application",
  },
};
...
export default function SignUp() {
  ...
  const inviteQuery = trpc.workflow.validateInvite.useQuery(
    { token: inviteToken! },
    { enabled: !!inviteToken }
  );
  const inviteData = inviteQuery.data as
    | {
        invite: {
  ...
  return (
    <AuthLayout
      eyebrow={isInviteFlow ? "You've been invited" : "Create an account"}
      title={
        isInviteFlow && inviteData
          ? `Join ${inviteOrgName} as ${inviteMemberRole === "faculty" ? "faculty" : "student"}`
          : "Be part of the solution."
      }
      description={
        isInviteFlow && inviteData
          ? `You've been invited to join ${inviteOrgName} as ${inviteMemberRole}. Create your account to accept the invite and access your ${inviteMemberRole} workspace.`
          : "Choose your role in the Samadhan network. Citizen participation begins with a challenge; institutions and industry partners continue to a verified organization profile."
      }
      footer={
    ...
    >
      {isInviteFlow ? (
        inviteQuery.isLoading ? (
          <div className="flex items-center gap-2 py-8 font-body text-[#52675d]">
            <span className="size-4 animate-spin rounded-full border-2 border-[#a78e6e] border-t-transparent" />
            Validating invite…
          </div>
        ) : inviteQuery.isError ? (
          <div className="border border-[#bd5a38]/60 bg-[#f7e2d6]/35 p-4">
            <p className="font-body text-[0.84rem] text-[#934325]">
              {(inviteQuery.error as Error).message}
            </p>
            <p className="mt-2 font-body text-[0.76rem] text-[#5d7067]">
              This invite link is invalid or expired. You can still create a
              regular account below.
            </p>
            <a
              href="/signup"
              className="mt-3 inline-block font-body text-[0.76rem] font-semibold text-[#b4401d] underline"
            >
          ...
          <div className="mb-6 border border-[#7ea68a] bg-[#e2ede3]/50 p-4">
            <p className="font-mono-ui text-[0.6rem] font-semibold uppercase tracking-[0.12em] text-[#2e5a3a]">
              Invite from {inviteOrgName}
            </p>
            <p className="mt-1 font-body text-[0.8rem] text-[#2e5a3a]">
              You are joining as{" "}
      ...
      <form onSubmit={submit} className="space-y-6">
        {!isInviteFlow && (
          <fieldset>
            <legend className="font-mono-ui text-[0.62rem] font-semibold uppercase tracking-[0.14em] text-[#25463a]">
              I&apos;m joining as
            </legend>
            <div className="mt-3 grid gap-3 sm:grid-cols-3">
              <AccountPathCard
                ...
                icon={<UserRound size={18} strokeWidth={1.45} />}
                {...roleCopy.citizen}
              />
              <AccountPathCard
                active={role === "institution"}
                onClick={() => setRole("institution")}
                icon={<Building2 size={18} strokeWidth={1.45} />}
                {...roleCopy.institution}
              />
              <AccountPathCard
                active={role === "industry"}
                onClick={() => setRole("industry")}
                icon={<BriefcaseBusiness size={18} strokeWidth={1.45} />}
                {...roleCopy.industry}
              />
            </div>
          </fieldset>
        ...
        {!isInviteFlow && (
          <div className="border-y border-[#a88d67]/40 py-5 font-body text-[0.79rem] leading-relaxed text-[#496258]">
            {role === "citizen"
              ? "Use a personal email and your home district so your reports can be routed to the right local context."
              : role === "institution"
                ? "Your institute application includes official contacts, academic capabilities, and the team that will manage faculty and students."
                : "Your industry application records the decision-makers, support modes, impact priorities, and capacity needed to match responsible partnerships."}
          </div>
        )}
        ...
        <FormField
          ...
          input={
            <input
              ...
              defaultValue={inviteData?.invite.email ?? ""}
              className="auth-input"
            />
          }
        />
        {!isInviteFlow && role === "citizen" && (
          <FormField
            label="District"
            input={
        ...
        <label className="flex items-start gap-3 pt-1 font-body text-[0.72rem] leading-relaxed text-[#436056]">
          <input
            type="checkbox"
            required
            className="mt-0.5 size-4 accent-[#d84a1b]"
          />
          <span>
        ...
        <button
          disabled={
            pending ||
            (isInviteFlow && (inviteQuery.isLoading || !!inviteQuery.isError))
          }
          type="submit"
...
function AccountPathCard({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`min-h-[8.4rem] rounded-2xl border p-4 text-left transition duration-200 ${active ? "border-[#d84a1b] bg-[#f8eee0] shadow-[inset_3px_0_0_#d84a1b]" : "border-[#a88d67]/55 bg-[#f7f0e5]/45 hover:border-[#6a856e] hover:bg-[#f8f2e8]"}`}
    >
      <span className={active ? "text-[#d84a1b]" : "text-[#355548]"}>
        {icon}
      </span>
      <span className="mt-3 block font-display text-[1.45rem] leading-none">
        {title}
      </span>
      <span className="mt-1 block font-body text-[0.66rem] leading-snug text-[#4b655b]">
        {description}
      </span>
    </button>
  );
  ...
  return (
    <label className="block">
      <span className="flex items-baseline justify-between font-mono-ui text-[0.62rem] font-semibold uppercase tracking-[0.13em] text-[#25463a]">
        {label}
        {helper && (
          <em className="font-body text-[0.66rem] normal-case tracking-normal text-[#6d8177]">
            {helper}
          </em>
        )}
      </span>
