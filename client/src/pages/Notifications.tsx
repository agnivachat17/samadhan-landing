      ...
      <section className="px-6 py-10 sm:px-10 lg:px-[5rem]">
        <div className="mx-auto max-w-[64rem]">
          ...
          {!email ? (
            ...
            <Empty label="No workflow notifications are recorded for this email." />
          ) : (
            <div className="mt-8 space-y-3">
              {query.data?.map(item => (
                <a
                  key={item.id}
                  href={item.href || "#top"}
                  className="block border border-[#a58c6d]/55 bg-[#f8f2e8]/25 p-5 hover:bg-[#e9dfcd]/50"
                >
                  <div className="flex items-start gap-4">
                    <Bell className="mt-1 text-[#c94a20]" size={19} />
                    <div>
                      <h2 className="font-display text-[1.5rem] leading-none">
                        {item.title}
                      </h2>
                      <p className="mt-3 font-body text-[0.8rem] leading-relaxed text-[#52675d]">
                        {item.body}
                      </p>
                      <p className="mt-3 font-mono-ui text-[0.52rem] uppercase tracking-[0.08em] text-[#64776d]">
                        {item.createdAt
