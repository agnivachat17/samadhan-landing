          ...
          {loading ? (
            ...
            <div className="mt-8 grid gap-8 xl:grid-cols-[minmax(0,1fr)_minmax(20rem,.55fr)]">
              <article>
                ...
                <section className="mt-8 border border-[#a58c6d]/55 bg-[#f8f2e8]/25 p-6">
                  ...
                  <div className="mt-5 flex items-center gap-3">
                    {organization ? (
                      <Building2 size={24} className="text-[#315947]" />
                    ) : (
                ...
                <section className="mt-8">
                  <p className="border-b border-[#a78e6e]/45 pb-3 font-mono-ui text-[0.6rem] uppercase tracking-[0.1em]">
                    Citizen challenge records
                  </p>
                  {citizenChallenges.length === 0 ? (
                    <p className="mt-5 font-body text-[0.78rem] text-[#607168]">
                      No citizen-reported challenge is associated with this
                      email.
                    </p>
                  ) : (
                    <div className="mt-5 space-y-3">
                      {citizenChallenges.map(item => (
                        <a
                          key={item.id}
                          href={`/admin/challenges/${item.id}`}
                          className="block border border-[#a58c6d]/45 p-4 hover:bg-[#e9dfcd]/50"
                        >
                          <p className="font-display text-[1.4rem] leading-none">
                            {item.title}
                          </p>
                          <p className="mt-2 font-body text-[0.72rem] text-[#5c7066]">
                            {item.status.replaceAll("_", " ")} · {item.district}
                          </p>
                        </a>
                      ))}
                    </div>
              ...
              <aside className="border border-[#a58c6d]/55 bg-[#f8f2e8]/25 p-6">
                <p className="font-mono-ui text-[0.6rem] uppercase tracking-[0.1em]">
                  Notification history
                </p>
                {(notificationsQuery.data ?? []).length === 0 ? (
                  <p className="mt-5 font-body text-[0.78rem] text-[#607168]">
                    No workflow notices recorded.
                  </p>
                ) : (
                  <div className="mt-5 space-y-3">
...
function Pair({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <dt className="font-mono-ui text-[0.52rem] uppercase tracking-[0.08em] text-[#64776d]">
        {label}
      </dt>
      <dd className="mt-2 font-body text-[0.76rem] leading-relaxed text-[#53675d]">
        {value}
      </dd>
    </div>
  );
