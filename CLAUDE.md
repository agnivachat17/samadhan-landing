...
- **`vercel.json` sets `Cross-Origin-Opener-Policy: same-origin-allow-popups` on all routes.** Firebase's `signInWithPopup` polls `popup.closed` on the opener; under a stricter COOP that read is blocked and Chrome logs `Cross-Origin-Opener-Policy policy would block the window.closed call` repeatedly. Don't tighten this to `same-origin` — it would break Google/Facebook sign-in detection.
- **`[Violation] Permissions policy violation: unload is not allowed in this document`** is harmless browser noise from an `unload` listener, not an app bug. Ignore it.
- **`api/_server.d.ts` is committed and must stay.** Vercel typechecks the function entrypoint, and `api/_server.js` is generated plain JS, so without this declaration the build fails with `TS7016: Could not find a declaration file for module './_server.js'` — which fails the whole deploy and silently leaves the *previous* (possibly broken) deployment live. A failed Vercel build looks exactly like "my fix didn't work": always confirm the deploy actually succeeded before debugging further.
