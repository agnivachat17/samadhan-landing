...
## USP-08 through USP-11, and the governance/ownership analysis — planning docs, not implemented

A research + architecture pass (separate from any code-implementation session) produced five new `docs/` files that describe **proposed** work, not shipped features — do not assume any of the following exists in the codebase just because a doc describes it in implementation-ready detail:

- `docs/USP-08-intelligent-routing.md` — problem-to-institution matching engine (the SIH problem statement explicitly asks for expertise-based routing; `enrollChallenge()`/`assignChallenge()` today are pure self-enrollment/manual-pick with no matching logic).
