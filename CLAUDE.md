...
A report came in that the Public Impact Ledger's "current status" was showing the wrong event (e.g. "Reported" as current on a challenge an institution had already enrolled for, accepted, and started work on). Investigated by pulling the *real* Firestore data for the actual reported challenge via the REST API (`https://firestore.googleapis.com/v1/projects/samadhan-sih/databases/(default)/documents/...`, anonymous, no credentials needed — same technique `tests/firestore.rules.test.ts` uses) rather than guessing from demo data. Found the real root cause: the challenge document's own `createdAt` (10:24:32) was numerically *later* than its self-enrolled assignment's `createdAt`/`updatedAt` (09:02:18 / 09:50:07) — i.e. two different Firestore collections' timestamps didn't agree on real-world ordering for this record. **Simply reversing the sort direction cannot fix this class of bug** — whichever event has the numerically largest timestamp will always land at whichever end of the list is called "latest," regardless of ascending vs. descending; the actual defect was trusting raw timestamp comparison across collections to mean "what stage is this challenge really at."

The fix: `TimelineEvent` now carries a `rank` (0=reported → 1=enrolled/assigned → 2=accepted → 3=project active → 4=outcome submitted → 5=citizen disputed → 6=citizen confirmed/resolved), assigned when each event is pushed based on the actual `challenge`/`assignment`/`project`/`closeout` **state fields** — not timestamps. "Current status" (`currentEvent`, shown in a dedicated always-visible hero card above the list, and used to place the pulsing "latest" marker + default-expanded row in the list below) is computed as the event with the highest `rank`, timestamp only used to break ties *within* the same rank. This is robust to any one collection's clock/write being off, and — per the same investigation — is also what makes the previously-existing "who's on it" panel (`InstitutionStatusPanel`, added in the self-enrollment fix above) and the timeline agree with each other, since both now ultimately derive from the same real state fields rather than one of them trusting a raw timestamp comparison the other doesn't.

**The chronological list itself still sorts by real timestamp, ascending (oldest at top, newest at bottom) — this part was never the bug** (verified independently against real seeded data before the report came in) and intentionally was *not* reversed; the user's own description of "correct order" ("reads naturally from the beginning of the workflow to the latest update") is literally ascending. Do not swap this sort to descending — it was checked against real data twice and matches the stated requirement. If a future report says the list itself looks wrong, re-verify against real Firestore data (per the REST technique above) before changing the sort direction, the same way this fix was actually found rather than guessed.

Also fixed in the same pass, both real (not cosmetic) bugs:
- **Every assignment now gets its own timeline event(s)**, not just `assignments[0]`. `db.ts`'s `listCollection` returns everything newest-first (`sortByCreatedAtDesc`), so `assignments[0]` was silently picking the *most recently created* assignment, not the first one — invisible with a single assignment (all of this session's earlier demo/manual testing), but wrong the moment more than one institution self-enrolls on the same challenge (a real, intentional feature — see self-enrollment section above). Assignments are now sorted ascending by `createdAt` and every one is rendered, plus a separate "accepted"/"declined" event when `assignment.status` resolves.
