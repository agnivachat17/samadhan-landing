...
- **`challenges`** - world-readable. Writes are scoped per field, not blanket `isSignedIn()`: the reporting citizen (`citizenEmail` match) may edit their own report content plus `status`/`resolutionSummary` (the fields `CitizenCloseoutConfirm.tsx`'s confirm/dispute flow needs); any signed-in user may cast/retract an upvote (`upvoteCount` only, via the existing transaction); and specifically **the institution holding a real, `accepted` assignment for _that exact challenge_** may flip `status` - checked with `isAcceptedAssignee()`, not just "is this account linked to some organization." That precision only works because assignment document ids are deterministic (see next bullet).
...
- `challenges.status` can be flipped by the institution holding a real accepted assignment for that specific challenge - but the rules cannot additionally verify _why_ that institution is writing `status` at that moment (e.g. as `createProject()`'s side effect vs. some other trigger), because there is no cheap way to distinguish call _intent_ in a security rule, only call _authorization_. This is accepted as sufficient - the authorization boundary (this institution, this challenge, a real accepted assignment) is what actually matters.
...
- No audit trail of _admin_ actions themselves exists (the USP-03 hash chain covers project activity/closeout writes, not who-approved-what on the admin side).
...
## USP-09, USP-10, USP-11, and USP-06 — since implemented (this section previously said otherwise; corrected)

An earlier pass through this repo found these described only in `docs/` planning files and documented them here as "not implemented." **That is now stale — all four have since been built and are live in the client.** Re-verified directly against the code (not against the docs) before writing this correction:

- **USP-09, assisted/multichannel reporting** — `SubmitChallenge.tsx` has a real assisted-submission mode (`isAssisted`/`kiosk`/`defaultAssisted` props): an operator can file on behalf of a beneficiary, capturing `beneficiaryName`/`beneficiaryPhone` and requiring consent, with a demo 6-digit OTP (hash stored, plain shown to the operator) issued for beneficiary confirmation. Challenges created this way carry `submittedVia: "assisted"` and `submittedByUid`.
- **USP-10, tiered/confidential visibility** — `SubmitChallenge.tsx` has a real `visibilityTier: "public" | "restricted" | "confidential"` selector. This is enforced in `firestore.rules`, not just hidden in the UI: `resource.data.get('visibilityTier', 'public') != 'confidential'` gates public reads of `challenges` (missing field defaults to `'public'`, so old records are unaffected), and the same check gates public reads of that challenge's `challengeEvidence`. `restricted` currently behaves the same as `public` for read purposes — see the "residual gap" note below.
- **USP-11, escalation** — `client/src/lib/analytics.ts` `computeEscalations()` extends the existing bottleneck detection into a staged ladder (14-day internal notice, 30-day external escalation) over `challenges`+`organizations`, rendered as a live banner on `AdminReports.tsx` and firing `createNotification()` once per session per newly-crossed threshold.
- **USP-06, academic credits & certificates** — `client/src/lib/db.ts` `awardCredits(projectId)` computes `creditsForProject(teamSize, milestoneCount)` (`min(100, teamSize*10 + milestones*5)`, from `client/src/lib/credits.ts`), writes `projects.creditsAwarded` and splits it evenly across `organizationMembers.creditsEarned`. `client/src/lib/certificate.ts` generates a verifiable PDF certificate client-side via lazy `jspdf`+`qrcode` imports (same lazy-import discipline as the ledger QR and OCR worker, to keep them out of the main bundle).

`docs/GOVERNANCE_OWNERSHIP_MODEL.md` is the one doc from that same research pass that is genuinely still just an analysis, not a feature — it has no corresponding code to implement (it's a recommendation about who should own/operate the platform post-launch, not a UI/data feature). Treat it as still aspirational; treat everything else in this section as shipped.

**Lesson for next time:** don't trust a CLAUDE.md status claim ("not implemented," "planning doc only") without grepping the actual code first — this file drifted out of date across sessions and nobody re-verified it before repeating the claim in the project README, which then had to be corrected too.
...
The SIH problem statement (ID 26043, `docs/Problem Statement ID.md`) explicitly requires "routing validated problem statements to appropriate universities based on their academic disciplines, research expertise... and faculty specialization." Before this, `assignChallenge()`/`enrollChallenge()` never looked at institutional fit at all — an admin picked from an alphabetical `<select>`, and an institution browsed an unordered list. This adds a client-side, pure-function match-score engine that ranks and explains fit, without gatekeeping either existing path.

- **`client/src/lib/matching.ts`** — `scoreInstitutionsForChallenge(challenge, organizations, assignments): MatchResult[]` (`{ organizationId, score: 0-100, reasons: string[] }`, sorted descending). Pure computation over already-fetched arrays, no Firestore calls of its own — same pattern as `client/src/lib/analytics.ts`. Only `kind === "institution" && verificationStatus === "verified"` organizations are scored. Four weighted signals: (1) keyword overlap between `challenge.domain`/`description` and `organization.departments`/`expertise`/`priorityDomains` (free-text, tokenized, >3-char words — same technique as `duplicateCheck.ts`'s `checkTitleDuplicate`), the strongest signal, up to 55 points; (2) geographic proximity via `findDistrictCentroid()` (`jharkhandDistricts.ts`, reused as-is — fuzzy substring match against both `challenge.district` and the institution's free-text `location` field) + haversine distance, capped at 25 points and tiered (not linear) so a strong keyword match from far away can still outrank a poorly-matched nearby institution — a deliberate counter to Ranchi's outsized share of university capacity, per the research cited in the USP-08 doc; (3) current active (`pending`/`accepted`) assignment load, up to 15 points, dampened as load rises, so fit doesn't pile every good match onto one popular institution; (4) a small 5-point nudge for having _any_ profile signal at all, so a blank profile doesn't tie with a filled one. Missing/empty fields contribute 0 to their signal, never a penalty — an institution that never filled in `expertise` is unranked, not excluded.
- **`client/src/pages/InstituteChallenges.tsx`** — the "Available challenges" grid is sorted by the selected institution's own fit score (`matchByChallengeId`, computed once against `[selectedInstitution]`). Cards scoring ≥65 get a filled dark-green "Strong fit for you" pill (`Target` icon), 35-64 get an outlined "Good fit for you" pill; below 35, **no badge at all** — deliberately, so the page doesn't become a wall of low-confidence percentages (a raw "12% fit" reads as noise, not signal). Clicking a badge expands a short bullet list of `reasons` (`AnimatePresence` height animation, same idiom as `ImpactTimeline`'s expand/collapse rows). If the selected institution has no `departments`/`expertise` at all, a dashed nudge card points at completing the profile instead of showing meaningless scores.
- **`client/src/pages/AdminChallengeDetail.tsx`** — a new "Suggested institutions" panel (top 3 scoring ≥35, `Target` icon, forest-green bordered card matching the site's other advisory-info-box styling) renders above the existing assignment form. Each suggestion is a clickable row (`motion.button`) showing name, `NN% fit`, and its top reason; clicking one sets the (unchanged, still-manual) `<select>`'s value — it does not submit anything. The `<select>`'s own options are also sorted by score and annotated (`"BIT Mesra — 82% fit"`). A second, unscoped `assignments.useQuery({})` was added (`allAssignmentsQuery`) alongside the existing challenge-scoped one, since the capacity signal needs each institution's _total_ active load across every challenge, not just this one. Assignment itself is completely unchanged — the admin still explicitly selects and clicks "Assign challenge"; this is a ranking/annotation layer only, never a gatekeeper.
- **No `firestore.rules` change** — only reads already-world-readable `organizations`/`challenges`/`assignments`. **No schema change** — `departments`/`expertise` stay free text for v1; a structured multi-select tag field is a documented fast-follow in the USP-08 doc, not required for this to work.
...
- **Verified end-to-end against the live Firestore project**, not just typecheck/build: a throwaway admin-SDK script (`firebase-admin`, using the same `FIREBASE_SERVICE_ACCOUNT_JSON` as `scripts/grant-admin.mjs`) created a verified test institution (`expertise: "water treatment, groundwater management, civil engineering, GIS"`, `location: "Ranchi"`) plus one well-matched Water/Ranchi challenge and one poorly-matched Livelihoods/Pakur challenge, then Playwright drove real logins through `/login` (not the extension) to confirm: the well-matched challenge sorted to the top of `/institute/challenges` with an expandable "Strong fit for you" badge showing accurate reasons; the poorly-matched one showed no badge; the admin's `/admin/challenges/:id` "Suggested institutions" panel ranked the same test institution first (and correctly ranked real seeded demo institutions below it by their own expertise text); clicking a suggestion synced the dropdown; mobile viewport (390px) and keyboard activation (`Tab`+`Enter` on the badge) both worked. All throwaway Firebase Auth users, `users/`, `organizations/`, and `challenges/` docs were deleted immediately after via a second cleanup script — nothing seeded for this QA pass was left in the live project.
...
## Student & faculty portal — institution accounts have real sub-roles

**Status: implemented** (design doc `STUDENT_FACULTY.md` at repo root is the original vision writeup; `STUDENT_FACULTY_IMPLEMENTATION.md` is the phase-by-phase build guide — both are historical implementation logs now, this section is the current source of truth).

An institution is no longer a single shared login. `client/src/lib/userProfile.ts` adds `MemberRole = "admin" | "faculty" | "student"` as an optional `memberRole` field on `UserProfile`, alongside the existing account-level `role: "institution"` — `memberRole` only exists for institution accounts; every other role treats it as absent.

- **`client/src/contexts/MemberRoleContext.tsx`** (`MemberRoleProvider`, mounted in `App.tsx`) resolves the current user's `memberRole` once via `trpc.auth.me` and exposes it through `useMemberRole()` / `useIsStudent()` / `useIsFaculty()` / `useIsInstitutionAdmin()`. A `null` memberRole (citizen/industry/admin accounts, or an institution account with no sub-role set) is treated as full ("admin") access by every caller — sub-roles are an added restriction on top of the institution role, not a replacement for it.
- **Onboarding is invite-token based, not admin-SDK-based** — there is no server, so an institution admin can't directly create a member's Firebase Auth account. Instead: the admin creates an `organizationInvites` record (`client/src/lib/db.ts`, `createOrganizationInvite`/`getInviteByToken`/`consumeInvite`, `~line 1295`) carrying `organizationId`+`memberRole`+an expiry, generates a link (`/signup?invite=<token>`), and the member signs up normally through that link — `consumeInvite` links their fresh account to the org and sets `memberRole` at that point. Invites expire and are single-use (`usedByUid` check).
- **Routes**: `/student/onboarding` + `/student/profile` (`StudentOnboarding.tsx`, `StudentProfile.tsx`) and `/faculty/onboarding` + `/faculty/profile` (`FacultyOnboarding.tsx`, `FacultyProfile.tsx`), plus `institute/StudentDashboard.tsx` / `institute/FacultyDashboard.tsx` both served at `/institute/dashboard` (the dashboard component itself branches on `useMemberRole()`, not a separate route, so the URL a member lands on after login is the same regardless of sub-role).
- **What students see**: their own projects (progress rings, milestone timelines), a live team-activity feed, a per-project forum, and an editable "My Skills" profile card (department/skills/year) — no enroll/create-project/edit-milestone controls, no other institution's data.
- **What faculty see**: every project they mentor or are a team member on (full activity timeline + document access), a student-oversight view of their team, the same project forum with mentor privileges (pin posts), and a read-only view of the institution's enrolled-challenge queue — same exclusions as students (no enroll, no create-project, no org-profile edit, no industry/funding visibility).
- **Route guarding**: `App.tsx`'s `instituteAdminGuarded()` wraps admin-only institute routes (challenges, challenge review, project workspace/closeout, projects list, profile) in an `AdminOnlyInstitute` component that reads `useMemberRole()` and redirects `student`/`faculty` back to `/institute/dashboard` if they land there directly — this is a **client-side-only** gate, consistent with the rest of the app's client-side role checks (see the "no server-side validation" limitation elsewhere in this file); the real enforcement boundary for what a student/faculty account can actually *write* is still whatever `firestore.rules` allows for a signed-in institution-linked account, which does not currently distinguish `memberRole` at all.
- **Project forum** (`client/src/components/ProjectForum.tsx` + `ForumPostCard.tsx`) is a flat per-project post list (`projectForumPosts` collection, `createForumPost`/`listForumPosts`/`updateForumPost`/`deleteForumPost` in `db.ts`) — one reply level, pinning, no `firestore.rules` distinction between student/faculty/admin authorship beyond the usual "must belong to this project's organization" ownership check.

**Known gap, not yet closed:** `firestore.rules` scopes writes to "is this account linked to this organization," the same boundary used before sub-roles existed — it does not yet distinguish `memberRole` at all. A student account that knows a project's id can currently call the same Firestore write paths a faculty/admin account can (create a milestone, edit a document) from the browser console; the sub-role restriction today is UI-only (the dashboard components simply don't expose those controls to students). If this needs real enforcement, it requires threading `memberRole` into the relevant `firestore.rules` write conditions the same way ownership was threaded through during the security-hardening pass described elsewhere in this file.

## Site footer — every link resolves to a real page, role-aware where it needs to be

**Status: implemented.** The footer previously rendered on `Home.tsx` only, and every one of its ~24 links was a dead `href="#top"` placeholder. It's now a shared component with every link pointing somewhere real.

- **`client/src/components/Footer.tsx`** — the single footer used everywhere (currently `Home.tsx` and every `/info/:slug` page). Don't recreate an inline footer on a new page; import this one, the same way `PublicPortalHeader` is reused for headers.
- **Role-aware Platform links, resolved the same way the rest of the app resolves role** (`trpc.auth.me` + `dashboardPathForRole` from `client/src/lib/roles.ts` — no new role logic invented): `Dashboard` and `Projects` resolve to the signed-in user's own dashboard/project list (citizen → `/citizen/dashboard`, institution → `/institute/projects` or onboarding if unverified, industry → `/industry/dashboard` or onboarding, admin → `/admin/projects`), and both fall back to `/login` when signed out. `Notifications` goes to `/notifications` (guarded for any role) or `/login`. `Reports` always points at `/admin/reports` — this is intentionally not hidden from non-admins; `ProtectedRoute`'s existing role mismatch behavior (redirect to the visitor's own dashboard) is what keeps a citizen from ever seeing admin content through it, exactly as it already does for any other admin-only route reached directly. **This is a UX convenience only — it doesn't change the access boundary**, which is still `ProtectedRoute` + `firestore.rules`, unchanged by this component.
- **`Register` (institutions) and `Partner with us` (industry) link to `/signup?role=institution` / `?role=industry`.** `SignUp.tsx` now reads a `role` query param on mount (`initialRole`, alongside the pre-existing `invite` param handling) to preselect the matching role tile — previously there was no query-param preselection at all, and `Home.tsx`'s own "I'm an institution" hero button had been silently pointing at the wrong, never-read param name (`?path=institution`); both now use `?role=`.
- **`client/src/pages/InfoPage.tsx` + `client/src/lib/infoContent.ts`** — the ~15 remaining footer items (How it works, Register-adjacent explainers, Submit solutions, Collaborate, Resources/Guidelines ×2 (institutions and industry are separate entries with separate content), Identify challenges, Offer solutions, Impact & scale, Support, Help center, Contact us, Privacy policy, Terms of use) are one generic template component (`PublicPortalHeader` + hero + prose sections + optional CTA + the shared `Footer`) driven by a single content data file, rather than 15 near-duplicate page files — consistent with the "don't over-split into many small files" convention, applied here as "don't over-split into many near-identical page *files*" rather than cramming unrelated content into one page. Route is the single dynamic `<Route path="/info/:slug" component={InfoPage} />` in `App.tsx`; an unknown slug renders the real `NotFound` page, not a blank screen. **Add new footer-linked static content here, not as a new page file.**
- **Content in `infoContent.ts` is written to be accurate about this specific codebase, not generic filler** — e.g. the privacy policy page names the actual third-party calls that leave the browser (Groq Vision for AI photo-categorization, MyMemory for live Hindi/Santali translation), not boilerplate "we may share data with partners" language. If a described feature changes, update the matching `infoContent.ts` entry in the same change, the same discipline as the rest of this file.
- **`SignUp.tsx`'s "I agree to Samadhan's terms of use and privacy policy" checkbox** previously linked both to `#top`; now links to `/info/terms-of-use` and `/info/privacy-policy` (`target="_blank"`, so agreeing doesn't navigate away from the half-filled signup form).
- **Deliberately no fabricated contact details anywhere** (`/info/contact-us`, `/info/support`) — no invented phone number, email address, or physical address. Both pages explain that in-platform contact (a challenge's own page, or an organization's verification status screen) is the real channel, rather than shipping a plausible-looking but fake official contact point.
- **`client/src/lib/siteContact.ts`** is the one place a real support email/phone number gets filled in later. Both fields ship empty. `InfoPage.tsx` only renders a "Reach us directly" block (`content.showContactChannels`, set on the `contact-us`/`support` entries in `infoContent.ts`) when at least one field is non-empty — so filling in `siteContact.ts` is enough to make the block appear on both pages, and leaving it empty (the current state) means neither page shows a phone/email section at all. Don't put a real or placeholder email/phone directly in `infoContent.ts` prose — route it through `siteContact.ts` so there is exactly one place to update it.
