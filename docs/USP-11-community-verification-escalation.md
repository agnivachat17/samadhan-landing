...
**Status:** Implemented. Corroboration/dispute support via `challengeSupports` kind extension, escalation notifications via `computeEscalations` + `createNotification`, escalation events in AdminReports bottleneck banner. See Implementation notes below; original design spec is preserved after it for context.
...
Samadhan already has upvoting (a simple "I care about this" signal) and a private admin bottleneck dashboard (a "someone should look at this" signal). USP-11 is not either of those renamed — it's (a) a _distinct-purpose_ corroboration signal that also lets citizens flag a report as stale/already-fixed (which upvoting cannot do), feeding real priority information rather than raw popularity, and (b) turning a passive internal dashboard into an _actual notification chain with a public consequence_, which is the missing "quality of resolution... not merely completed" accountability the research explicitly says CPGRAMS-style disposal-rate metrics alone don't provide.
