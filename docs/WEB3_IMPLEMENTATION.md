...
- We already have a hash ledger (USP-03) — the NFT is an _additional_ trust layer, not a replacement
...
**Recommendation:** Option A for the demo. The NFT is _for_ the student, not _by_ the student. The platform mints it on their behalf. Users don't need to know about wallets, gas, or blockchain — they just see "Your credential is now on the public ledger" with a QR code.
