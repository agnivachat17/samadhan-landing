...
export async function awardCredits(
  ...
  for (const m of members) {
    await updateRecord(collectionNames.organizationMembers, m.id, {
      creditsEarned: (m.creditsEarned ?? 0) + share,
    });
  }
  return { credits };
}
...
export async function generateCertificate(input: {
  ...
  doc.setFontSize(12);
  doc.text("Government of Jharkhand · Civic Innovation", 15, 32);
  doc.setFontSize(18);
  doc.text(input.projectTitle, 15, 55);
  doc.setFontSize(11);
  doc.text(
  ...
  doc.text(`${input.credits} credits awarded`, 15, 85);
  doc.setFontSize(9);
  doc.setTextColor(82, 103, 93);
  doc.text(
    `Date: ${input.date.toLocaleDateString()}  ·  Anchor: ${input.anchorRoot.slice(0, 24)}…`,
    15,
    95
  );
  // QR bottom-right
  ...
  doc.setFontSize(7);
  doc.text("Scan to verify ledger root", 245, 180);
  // seals
...
- **`InstituteProjectWorkspace.tsx:173 Delivery control`** — when `stage==="closeout" && status==="resolved" && !project.creditsAwarded` show:
  ...
  <button
    onClick={async () => {
      const { credits } = await awardCredits.mutateAsync({
    ...
    className="rounded-full bg-[#16422f] px-5 py-3 font-mono-ui text-[0.58rem] uppercase text-white"
  >
    Award credits & certificate
  </button>
  ```
